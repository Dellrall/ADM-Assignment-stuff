...
SELECT 
    ...
    RPAD(NVL(v.Brand, 'Generic'), 16) AS "Brand",
    v.DamagedUnits AS "Damaged",
    v.DefectiveUnits AS "Defective",
    v.ExpiredUnits AS "Expired",
    (v.DamagedUnits + v.DefectiveUnits + v.ExpiredUnits) AS "Total Flawed Units",
    TO_CHAR(v.TotalMonetaryLoss, 'FM99,990.00') AS "Total Loss (MYR)",
    CASE 
        WHEN v.TotalMonetaryLoss > 200 THEN 'CRITICAL SUPPLIER AUDIT'
        WHEN v.TotalMonetaryLoss > 50 THEN 'MONITOR BATCH QUALITY'
...
SELECT 
    ...
    SUM(CASE WHEN r.RefundStatus = 'Approved' THEN 1 ELSE 0 END) AS "Approved",
    SUM(CASE WHEN r.RefundStatus = 'Rejected' THEN 1 ELSE 0 END) AS "Rejected",
    SUM(CASE WHEN r.RefundStatus = 'Pending' THEN 1 ELSE 0 END) AS "Pending",
    TO_CHAR(NVL(SUM(r.RefundAmount), 0), 'FM99,990.00') AS "Total Payout (MYR)",
    ROUND((SUM(CASE WHEN r.RefundStatus = 'Approved' THEN 1 ELSE 0 END) / NULLIF(COUNT(DISTINCT r.RefundID), 0)) * 100, 2) || '%' AS "Approval Rate"
FROM Branch b
LEFT JOIN CustomerOrder o ON b.BranchID = o.BranchID
LEFT JOIN Refund r ON o.OrderID = r.OrderID
GROUP BY b.BranchID, b.BranchName
ORDER BY COUNT(DISTINCT r.RefundID) DESC;
