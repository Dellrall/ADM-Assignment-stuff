...
| **Task 7** | [`04_reports.sql`](04_reports.sql) | • **Report 1:** `sp_rpt_active_promotions_catalog` (Nested cursors: Active Campaigns $\rightarrow$ Enrolled Grocery Items & Savings %).<br>• **Report 2:** `sp_rpt_promotion_sales_impact` (Nested cursors: Campaign Header $\rightarrow$ Order Transactions & Customer Net Revenue). |
